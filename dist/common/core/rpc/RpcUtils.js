...
exports.RpcUtils = exports.RpcUtilsByBuffer = exports.RpcUtilsByJson = exports.RpcMessageType = void 0;
...
class RpcUtilsByJson {
    ...
    static transferMessage(jsonStr, serverMapList, nodeIdMap) {
        ...
        switch (reqMsg.type) {
            ...
            case RpcMessageType.send:
                // 转发
                const clients = RpcUtilsByJson.getRouteClient(reqMsg, serverMapList, nodeIdMap);
                clients === null || clients === void 0 ? void 0 : clients.forEach((c) => {
                    c.socket.send(jsonStr);
                });
                break;
            case RpcMessageType.result:
                (_a = nodeIdMap.get(reqMsg.fromNodeId)) === null || _a === void 0 ? void 0 : _a.socket.send(jsonStr);
                break;
        }
    }
    /** 序列化call请求 */
    ...
    /** 获取转发的clients */
    static getRouteClient(reqMsg, serverMapList, nodeIdMap) {
        if (reqMsg.routeOptions.type === StoneDefine_1.RpcRouteType.Target /* target */) {
            ...
            if ((result === null || result === void 0 ? void 0 : result.serverType) !== reqMsg.serverName) {
                return [];
            }
            return [result];
        }
 ...
 * 详情测试报告运行npm test查看
 **/
class RpcUtilsByBuffer {
    /** 序列化参数列表 */
    static encodeArgs(buffer, offset, args) {
    ...
    /** 序列化参数 */
    static encodeArg(buffer, offset, arg) {
        switch (typeof (arg)) {
            case 'string':
                {
    ...
    /** 反序列化参数列表 */
    static decodeArgs(buffer, offset) {
        const args = [];
        while (true) {
    ...
    /** 反序列化参数 */
    static decodeArg(buffer, offset = 0) {
        let result;
    ...
    /** 序列化call请求 */
    static encodeCallReqest(nodeId, serverName, className, funcName, requestId, routeOptions, args) {
    ...
    /** 序列化send请求 */
    static encodeSendReqest(nodeId, serverName, className, funcName, routeOptions, args) {
        return RpcUtilsByBuffer.encodeReqMsg({
            type: RpcMessageType.send,
            routeOptions,
            serverName,
    ...
    static getRpcMsgType(buffer) {
        return buffer.readUint8();
    }
    /** 写入路由信息 */
    static writeRouteOptions(routeOption, buffer, offset) {
    ...
    /** 读取roter信息 */
    static readRouteOptions(routeOption, buffer, offset = 9) {
    ...
    static getRouteClient(buffer, serverMapList, nodeIdMap) {
        ...
        if (routeOptions.type === StoneDefine_1.RpcRouteType.Target /* target */) {
            const result = nodeIdMap.get(routeOptions.nodeId);
            if ((result === null || result === void 0 ? void 0 : result.serverType) !== serverName) {
                return [];
            }
            return [result];
        }
        else if (routeOptions.type === StoneDefine_1.RpcRouteType.All /* all */) {
    ...
    /** 转发rpc信息 */
    static transferMessage(buffer, serverMapList, nodeIdMap) {
        var _a;
        ...
        switch (type) {
            ...
            case RpcMessageType.result:
                ...
                (_a = nodeIdMap.get(nodeId)) === null || _a === void 0 ? void 0 : _a.socket.send(buffer);
                break;
        }
    }
    /** 反序列化rpc 信息,返回rpc请求或结果 */
    static decodeRpcMsg(buffer) {
...
exports.RpcUtilsByBuffer = RpcUtilsByBuffer;
RpcUtilsByBuffer._randIndex = 1;
RpcUtilsByBuffer._sourceBuffer = Buffer.alloc(65536);
